...
'TTR', 'xtable', 'multcomp', 'pander', 'png', 'grid', 'tseries', 'nlme', 'forecast', 'lsmeans', 'ape', 'seqinr', 'phylobase', 'adephylo', 'geiger', 'picante', 'stats', 'RColorBrewer', 'caper', 'phylolm', 'pmc', 'phangorn', 'pander')
...
> NJ is a useful starting tree because it is simple to understand and straightforward. Disadvantages could include that it must be ultrametric; doesn't consider Bayesian probabilities; I think that it assumes constant rate of evolution on all branches (but I could be wrong).
...
```{r}
#1
seq.dist.GTR <- dist.dna(p.DNAbin, model = "GTR", pairwise.deletion = FALSE)

#START HERE DUDE
#2
par(mar = c(5, 5, 2, 1) + 0.1)
plot(seq.dist.GTR, seq.dist.F84, pch = 20, col = "red", las = 1, asp = 1, xlim = c(0, 0.7), ylim = c(0, 0.7), xlab = "Raw Distance", ylab = "F84 Distance")
abline(b = 1, a = 0, lty = 2)
text(0.65, 0.6, "1:1")
