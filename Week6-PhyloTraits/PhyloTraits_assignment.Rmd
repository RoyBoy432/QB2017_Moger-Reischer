...
> NJ is a useful starting tree because it is simple to understand and straightforward. Disadvantages could include that it must be ultrametric; doesn't consider Bayesian probabilities; I think that it assumes constant rate of evolution on all branches (but I could be wrong); it must assume that the distance matrix is true.
...
table.phylo4d(x, treetype = "cladogram", symbol = "colors", show.node = TRUE,cex.label = 0.5, scale = FALSE, use.edge.length = FALSE,edge.color = "#f527b7", edge.width = 1.5, box = FALSE, col=mypalette(20), pch = 15, cex.symbol = 1.25, ratio.tree = 0.2, cex.legend = 1.5, center = FALSE)

#3
par(mar=c(1,5,1,5) + 0.1)
x.nb <- phylo4d(nj.rooted, nb, missing.data="OK")
table.phylo4d(x.nb, treetype = "clad", symbol = "colors", show.node = TRUE, cex.label = 0.4, scale = FALSE, use.edge.length = FALSE, edge.color = "#2c519d", edge.width = 2, box = FALSE, col=mypalette(25), pch = 15, cex.symbol = 1.25, var.label=("NB"), ratio.tree = 0.5, cex.legend = 1.5, center = FALSE)
...
> Consider anammox bacteria, which fix nitrogen using a highly unstable molecule, hydrazine (N2 H4). These bacteria must invest in an anammoxosome, a special subcellular compartment, to store its volatile supply of hydrazine. However, although specialist bacteria must invest additional resources to be able to carry their method of N fixation, they have the advantage of being able to grow in certain environments which would be inhospitabal to species that could not handle hydrazine and would not be able to obtain N.
...
LLTRADE<-filter(tradeoff, lake=="LL")
WGTRADE<-filter(tradeoff, lake=="WG")
...
#scatterplot
#plot(abline(fitL))
#ggplot(tradeoff,aes(nb,log10(umax)))+ geom_quantile(x=nb,y=log10(umax),color=lake) + geom_point(x=nb,y=log10(umax),color=lake)
...
plot(LLTRADE$NB, log10(LLTRADE$umax), las = 1, xlab = "Niche breadth", ylab = "log umax",col="#5b57af", pch=8,cex=1.5)
...
plot(WGTRADE$NB, log10(WGTRADE$umax), las = 1, xlab = "Niche breadth", ylab = "log umax",col="#54d5d7", pch=8,cex=1.5)
